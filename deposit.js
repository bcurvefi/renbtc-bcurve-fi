...
async function handle_sync_balances() {
    ...
    if (max_balances) {
        ...
        for (let i = 0; i < N_COINS; i++) {
            var val = Math.floor(wallet_balances[i] * c_rates[i] * 100000000) / 100000000;
            $('#currency_' + i).val(val.toFixed(8));
        }
    } else {
...
async function handle_add_liquidity() {
    ...
    for (let i = 0; i < N_COINS; i++) {
        ...
        if(Math.abs(balance/amount-1) < 0.00000001) {
                ...
                for (let j = 0; j < N_COINS; j++)
                    if (j != i) {
                        ...
                        if (balances[i] * c_rates[i] > 1) {
                            ...
                            newval = Math.floor(newval * c_rates[j] * 100000000) / 100000000;
                            el_j.val(newval);

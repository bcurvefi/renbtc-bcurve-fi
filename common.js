...
async function ensure_underlying_allowance(i, _amount) {
    ...
    var current_allowance = cBN(await coins[i].methods.allowance(default_account, swap_address).call());
    ...
    if ((current_allowance.isGreaterThan(cBN(0))) & (current_allowance.isLessThan(amount)))
        await approve(coins[i], 0, default_account);
    return await approve(coins[i], cBN(amount).toFixed(0,1), default_account);
}
...
async function init_contracts() {
    ...
    for (let i = 0; i < N_COINS; i++) {
        ...
        coins[i] = new web3provider.eth.Contract(ERC20_abi, addr);
    }
}
...
async function update_fee_info() {
    ...
    resolves.forEach((balance, i) => {
        balances[i] = +balance;
        $(bal_info[i]).text((balances[i] * c_rates[i]).toFixed(8));
        total += balances[i] * c_rates[i];
    })
    $(bal_info[N_COINS]).text(total.toFixed(8));
    fee = parseInt(await swap.methods.fee().call()) / 1e10;
    ...
    if (default_account) {
        ...
        if (token_balance > 0) {
            ...
            for (let i=0; i < N_COINS; i++) {
                ...
                total += val;
                $(l_info[i]).text(val.toFixed(8));
            }
            $(l_info[N_COINS]).text(total.toFixed(8));
